...
router.get('/', async (req, res) => {
    ...
    async function createSocketSession() {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                try {
                    ...
                    const successMsg = {
                        text:
                            `🚀 *REDZONE-MD Session Created!*\n\n` +
                            `▸ *Never share* your session ID\n` +
                            `▸ Join our WhatsApp Channel\n` +
                            `▸ Report bugs on GitHub\n\n` +
                            `_Powered by REZONE-MD_\n\n` +
                            `🔗 *Useful Links:*\n` +
                            `▸ GitHub: https://github.com/QadeerXTech/REDZONE-MD\n` +
        ...
        if (!sock.authState.creds.registered) {
            await delay(1500);
            const pairingCode = await sock.requestPairingCode(phoneNumber, "REDZONE123");
            if (!res.headersSent) {
